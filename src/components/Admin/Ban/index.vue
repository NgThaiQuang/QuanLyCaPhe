            ...
            <div class="col-4">
                <div class="card">
                    <div class="card-header bg-info ">
                        <h4 class="card-title text-white mt-2 text-center">Thêm bàn mới</h4>
                    </div>
                    <div class="card-body">
                        <label for="" class="mt-2 form-label">Khu vực bàn</label>
                        ...
                        <label for="tenBan" class="form-label mt-2">Tên bàn</label>
                    ...
                    <div class="card-footer d-flex">
                        <button @click="themBan()" class="btn btn-primary text-end ms-auto">+ Thêm bàn</button>
                    </div>
                </div>
            </div>
            <div class="col-8">
                <div class="card">
                    <div class="card-header bg-info ">
                        <h5 class="card-title text-white mt-2 text-center">Danh Sách Bàn</h5>
                    </div>
                    <div class="card-body">
                        <div class="container">
                            ...
                            <div class="table-reponsive">
                                <table class="table table-striped table-bordered align-middle">
                                    <thead class="bg-primary text-light">
                                        <th class="text-center py-2" style="width:10%">#</th>
                                        <th class="text-center py-2" style="width:15%">Tên Bàn</th>
                                        <th class="text-center py-2" style="width:15%">Khu Vực</th>
                                        <th class="text-center py-2" style="width:40%">Tình Trạng</th>
                                        <th class="text-center py-2" style="width:20%">Hành Động</th>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(value, index) in danhSachLoc()" :key="index">
                                            ...
                                            <td class="text-center">
                                                <button class="btn btn-success w-100"
                                                    :class="value.tinh_trang == 1 ? 'btn-secondary' : 'btn-danger'">{{
                                                    value.tinh_trang == 1 ? 'Hoạt Động' : 'Tạm Tắt' }}</button>
                                            </td>
...
export default {
    ...
    methods: {
        loadData() {
            axios.get('https://bek26.deloydz.io.vn/api/admin/ban/get-data').then((res) => {
                this.list = res.data.data;
            })
        },
        dieuKienLoc(value) {
            let tuKhoa = this.tuKhoa.toLowerCase().trim();
            return value.ten_ban.toLowerCase().includes(tuKhoa) || value.ten_khu_vuc.toLowerCase().includes(tuKhoa) || value.tinh_trang.toLowerCase().includes(tuKhoa);
        },
        danhSachLoc() {
